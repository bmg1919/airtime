...
apt-get -y install libopus0 libopus-dev libopus-dbg libopus-doc
...
apt-get -y install git-core ocaml-findlib libao-ocaml-dev \
...
if [ "$code" != "lucid" ]; then
    apt-get -y install libvo-aacenc-dev
fi
...
#for aac+
#rm -rf libaac*
#apt-get -y install libfftw3-dev pkg-config autoconf automake libtool unzip
